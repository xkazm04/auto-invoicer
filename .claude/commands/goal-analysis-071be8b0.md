# QR payment codes (SPD) + invoice detail view from dashboard

## Goal
[AUTONOMOUS - Phase 2a, Business Domain Scan] Two highest-impact gaps: (1) Czech SPD QR payment codes on invoices � auto-generated from IBAN/amount/reference, rendered in form preview and PDF. Every Czech competitor has this. (2) Clickable dashboard rows opening read-only invoice detail view with status update. Source: domain scan #1 + open follow-up.
