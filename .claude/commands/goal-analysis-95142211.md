# Full form polish: validation, line-item management, editable metadata, localStorage drafts

## Goal
Combined goal covering all Run #1 open follow-ups: (1) zod validation with inline errors, (2) remove line item + reorder, (3) editable invoice number and status workflow (draft/sent/paid/overdue), (4) localStorage persistence with Save Draft wired up, drafts list, and Create Invoice gated on validation. Ambitious single-goal iteration to stress-test the skill on larger scope.
