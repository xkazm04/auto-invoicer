...
function computeKpis(
  archived: ArchivedInvoiceSummary[],
  drafts: DraftSummary[]
): KpiData {
  let totalInvoiced = 0;
  let totalPaid = 0;
...
export default function DashboardPage() {
  ...
  const kpis = computeKpis(archived, drafts);
  ...
  // Combine archived + drafts into a unified list
  type UnifiedInvoice = {
    id: string;
    number: string;
    status: string;
  ...
  const kpiCards: { label: string; monoLabel: string; value: string; accent: string }[] = [
    {
      label: "Total Invoiced",
      monoLabel: "invoiced",
      value: formatMoney(kpis.totalInvoiced, kpis.currency, { decimals: false }),
      accent: isMono ? "text-neutral-900" : "text-neutral-800",
    },
    {
      label: "Paid",
      monoLabel: "paid",
      value: formatMoney(kpis.totalPaid, kpis.currency, { decimals: false }),
      accent: isMono ? "text-neutral-900" : "text-emerald-700",
    },
    {
      label: "Outstanding",
      monoLabel: "outstanding",
      value: formatMoney(kpis.totalOutstanding, kpis.currency, { decimals: false }),
      accent: isMono ? "text-neutral-900" : "text-amber-700",
    },
    {
      label: "Overdue",
      monoLabel: "overdue",
      ...
      accent: kpis.overdueCount > 0
        ? (isMono ? "text-neutral-900" : "text-red-600")
        : (isMono ? "text-neutral-400" : "text-neutral-400"),
    },
  ];
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className={`grid grid-cols-4 ${isMono ? "gap-px bg-neutral-200 mb-8" : "gap-4 mb-8"}`}>
        {kpiCards.map((card) => (
          <div
            key={card.label}
            className={`${t.cardBg} ${t.cardRadius} ${isMono ? "p-3" : "p-5"} ${t.cardShadow}`}
          >
      ...
      {filtered.length === 0 ? (
        ...
        <div className={`${isMono ? "" : "space-y-2"}`}>
          ...
          <div className={`grid grid-cols-12 gap-4 ${isMono ? "pb-2 border-b border-neutral-200" : "px-4 pb-2"}`}>
            <div className={`col-span-2 text-[${isMono ? "9px" : "10px"}] font-semibold ${t.labelColor} uppercase tracking-wider`}>
              Number
            </div>
            <div className={`col-span-3 text-[${isMono ? "9px" : "10px"}] font-semibold ${t.labelColor} uppercase tracking-wider`}>
              Customer
            </div>
            <div className={`col-span-2 text-[${isMono ? "9px" : "10px"}] font-semibold ${t.labelColor} uppercase tracking-wider`}>
              Date
            ...
            <div className={`col-span-2 text-[${isMono ? "9px" : "10px"}] font-semibold ${t.labelColor} uppercase tracking-wider text-right`}>
              Amount
            </div>
            <div className={`col-span-1 text-[${isMono ? "9px" : "10px"}] font-semibold ${t.labelColor} uppercase tracking-wider text-right`}>
              Status
            </div>
          </div>
          ...
          {filtered.map((inv) => {
            ...
            return (
              <div
                ...
                className={`grid grid-cols-12 gap-4 items-center ${t.cardBg} ${t.cardRadius} ${isMono ? "py-2" : "px-4 py-3"} ${t.cardShadow} ${t.cardHoverShadow} transition-shadow`}
              >
                <div className={`col-span-2 ${isMono ? "text-[13px] font-medium" : "text-sm font-medium"} ${t.inputText}`}>
                  #{inv.number}
                </div>
                <div className={`col-span-3 ${isMono ? "text-[13px]" : "text-sm"} ${t.inputText} truncate`}>
                  {inv.customerName || (inv.source === "draft" ? "Draft" : "—")}
                </div>
                <div className={`col-span-2 ${isMono ? "text-[13px]" : "text-xs"} ${t.labelColor}`}>
                  {formatDate(inv.issueDate)}
                </div>
                <div className={`col-span-2 ${isMono ? "text-[13px]" : "text-xs"} ${overdue ? "text-red-600 font-medium" : t.labelColor}`}>
                  {formatDate(inv.dueDate)}
                  {overdue && !isMono && <span className="ml-1 text-red-500">!</span>}
                </div>
